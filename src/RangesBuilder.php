...
class RangesBuilder 
{
		...
		for($i = 1; $i <= $len; $i++) {
			if(array_key_exists($i, $eIndex)) {	
				foreach($eIndex[$i] as $range) {
					$s = $range[S];
					if($path[$s] !== null) {
						$info = array($s, $path[$s][WEIGHT] + 1, $path[$s][PATH_UNK], $range[LINK_TYPE]);
	...
	private function pathToRanges($path, $len) {
		...
		while($i > 0) {
			$info = $path[$i];
			$s = $info[POINTER];
			$ranges[] = array($s, $i, $info[PATH_LINK_TYPE]);
			$i = $s;
		}		
